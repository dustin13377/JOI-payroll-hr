-- Fix: "column reference invoice_id is ambiguous" thrown by generate_weekly_invoices,
-- which made the "Generate" button fail with HTTP 400.
--
-- The function's RETURNS TABLE declares an output column `invoice_id`, which shadows
-- invoice_lines.invoice_id in the final post-spiffs total re-read. Postgres 17 can't
-- tell which one is meant and raises "ambiguous". Qualify the column as
-- invoice_lines.invoice_id. No logic change — only that one WHERE clause is disambiguated.
...
BEGIN
  ...
  LOOP
    ...
    -- Qualify invoice_lines.invoice_id to avoid shadowing the RETURNS TABLE output column.
